...
function createSearch(queries){
  ...
  if(queries.searchType && queries.searchText && queries.searchText.length >= 3){
    ...
    if(searchType.indexOf("body")>=0){
      postQueries.push({body:{ $regex : new RegExp(queries.searchText, "i")}}); // CHECK THE ERROR : new REgExp 라고 썼다가 검색창에 숫자넣으니 에러.

    }
    if(postQueries.length > 0) findPost = {$or:postQueries};
  }
  return { searchType : queries.searchType, searchText:queries.searchText, findPost:findPost};
}
